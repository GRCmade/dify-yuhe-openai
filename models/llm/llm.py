...
class OpenAILargeLanguageModel(_CommonOpenAI, LargeLanguageModel):
    ...
    def validate_credentials(self, model: str, credentials: dict) -> None:
        ...
        try:
            ...
            if credentials.get("api_protocol") == "responses":
                self._validate_responses_api_credentials(client=client, model=model)
    ...
    def _validate_responses_api_credentials(self, client: OpenAI, model: str) -> None:
        response_params = {
            "model": model,
            "input": [
                {
        ...
        try:
            client.responses.create(**response_params)
            return
        except Exception as ex:
            if not isinstance(ex, (openai.InternalServerError, openai.APIConnectionError)):
                raise

        # Some OpenAI-compatible gateways only support streaming /v1/responses.
        stream = None
        try:
            stream = client.responses.create(**response_params, stream=True)
            for _ in stream:
                break
        finally:
    ...
    def _build_responses_api_params(
        ...
        # Some OpenAI-compatible Responses gateways backed by the stricter
        # ChatGPT Codex endpoint reject these otherwise-public fields.
        for unsupported_field in ("metadata", "presence_penalty", "frequency_penalty"):
            params.pop(unsupported_field, None)
    ...
    def _convert_prompt_messages_to_responses_input(
        ...
        for m in prompt_messages:
            if isinstance(m, SystemPromptMessage):
                # Some OpenAI-compatible Responses gateways backed by the
                # stricter ChatGPT Codex endpoint reject role="system", but
                # accept role="developer" / top-level instructions.
                input_items.append({
                    "type": "message",
                    "role": "developer",
